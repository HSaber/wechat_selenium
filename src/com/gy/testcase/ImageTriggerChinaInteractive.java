...
public class ImageTriggerChinaInteractive extends TestBase{
	private static Logger logger = Logger.getLogger(ImageTriggerChinaInteractive.class);
	//@Test
	@Test(groups="imageTriggerChinaInteractive",dependsOnGroups="imageTrigger48Interactive")
	  public void imageTriggerChinaInteractive() throws Exception {
		navigation("Triggers");
	  	  Thread.sleep(2000);
	        ...
	        System.out.println("接口post操作前，ImageTriggerUpdateChina当前总触发次数是"+a+"次");
	        ...
	        System.out.println("接口post前，rainbow触发ImageTriggerUpdateChina的次数是:"+c);
		  ...
		  //String[] info= {"appid","secret","mid","WechatID","openid"};
			    ...
			    if(category2.equals("seleniumchina image trigger")){
		  	    ...
		  	    try {
		  	        ...
		  	        System.out.println("接口post操作后，ImageTriggerUpdateChina当前总触发次数是"+num+"次");
		  	    	AssertJUnit.assertTrue(num>=a);
		  	      } catch (Error e) {
		  	        //verificationErrors.append(e.toString());
		  	    	//System.out.println("MTP trigger更改segmentation后触发次数显示错误，更改前没触发成功！！"); 
		  	    	logger.error("ImageTriggerUpdateChina搜索失败，请检查！！");
		  	        Assert.fail("ImageTriggerUpdateChina搜索失败，请检查！！");
		  	      }
		  	    ...
		  	    try {
		  	      AssertJUnit.assertEquals(c,r);
		  	    	...
		  	    	if(r>c){
		  	    		logger.error("rainbow不应该触发ImageTriggerUpdateChina，请检查！！");
		  	    		Assert.fail("rainbow不应该触发ImageTriggerUpdateChina，请检查！！");
		  	    	}else{
		  	    		logger.error("ImageTriggerUpdateChina搜索失败，请检查！！");
		  	    		Assert.fail("ImageTriggerUpdateChina搜索失败，请检查！！");
		  	    	}
