...
public class WelcomeImageInteractive extends TestBase{
	private static Logger logger = Logger.getLogger(WelcomeImageInteractive.class);
	 @BeforeMethod
	...
	public void tearDown() throws Exception {
	  ...
	  @Test(dependsOnGroups="welcomeImage",groups="welcomeImageInteractive")
		public void welcomeImageInteractive() throws Exception {
		//接口部分
		...
		Element root = doc.getRootElement();
	   	...
	   	if(it.hasNext()){
	   		...
	   		while(it.hasNext()){
		   		...
		   		System.out.println("welcome image得到的MediaId是"+mediaid);
		   		try{
		   				assertNotNull(mediaid);
		   				System.out.println("welcome image触发image 成功 ！！！");
		   				
		   		}catch(Error e){
		   			
		   			logger.error("welcome image触发image失败！！！");
		   			Assert.fail("welcome image触发image失败！！！");
		   		}
		   	}
	   	
	   	}else{
	   		logger.error("welcome image触发失败,xml没找到image");
	   		Assert.fail("welcome image触发失败,xml没找到image");
	   	}
