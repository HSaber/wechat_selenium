...
public class KeywordImage extends TestBase{
	private static Logger logger = Logger.getLogger(KeywordImage.class);
	@Test(groups="keywordImage")
	  public void keywordImage() throws Exception {
	    ...
	    driver.findElement(By.id("KeyWord_category")).sendKeys("seleniumkeywordimage");
	    driver.findElement(By.id("KeyWord_name")).clear();
	    driver.findElement(By.id("KeyWord_name")).sendKeys("keywordimageselenium");
	    driver.findElement(By.id("KeyWord_internal_name")).clear();
	    driver.findElement(By.id("KeyWord_internal_name")).sendKeys("keywordimageselenium");
	    driver.findElement(By.cssSelector("input.keyword")).clear();
	    driver.findElement(By.cssSelector("input.keyword")).sendKeys("seleniumimage");
	    ...
	    driver.findElement(By.cssSelector("input.tagInputField.ui-autocomplete-input")).sendKeys("seleniumimage" + time);
	    Thread.sleep(1000);
	    ...
	    String search = "";
	    boolean b = true;
	   /* while(!search.equals("seleniumimage")){
        ...
        if (breakIt&&search.equals("seleniumimage")) {
            break;
        }
	    ...
	    while(true){
	    	...
	    	try{
	    		...
	    		if(search.equals("seleniumimage")&&b)
	    			break;
	    }
	    
	    try {
		      AssertJUnit.assertEquals("seleniumimage", search);
		    } catch (Error e) {
		    	logger.error("创建keywordimage，list页面keyword显示错误，或者创建失败，请检查！！！");
		    	Assert.fail("创建keywordimage，list页面keyword显示错误，或者创建失败，请检查！！！");
		    }
	    ...
	    try {
	      AssertJUnit.assertEquals("seleniumkeywordimage", search);
	    } catch (Error e) {
	    	logger.error("创建keywordimage，list页面category显示错误，或者创建失败，请检查！！！");
	    ...
	    try {
	      AssertJUnit.assertEquals("keywordimageselenium", search);
	    } catch (Error e) {
	    	logger.error("创建keywordimage，list页面name显示错误，或者创建失败，请检查！！！");
	    	Assert.fail("创建keywordimage，list页面name显示错误，或者创建失败，请检查！！！");
	    }
	    ...
	    try {
	      AssertJUnit.assertEquals("Image", search);
	    } catch (Error e) {
	    	System.out.println("创建keywordimage，list页面type显示错误，或者创建失败，请检查！！！");
	    	Assert.fail("创建keywordimage，list页面type显示错误，或者创建失败，请检查！！！");
	    }
	    
	    while(true){
	    	b=true;
	    	try{
	    		search = driver.findElement(By.xpath("//div[@id='key-word-grid']/div/table/tbody/tr/td[6]/img")).getAttribute("src");
	    	}catch(org.openqa.selenium.StaleElementReferenceException e){
	   		 	System.out.println("搜索中，捕捉异常org.openqa.selenium.StaleElementReferenceException");
	   		 	b=false;
	    	}
	    ...
	    try {
	    	...
	    	logger.error("创建keywordimage，list页面不显示图片，或者创建失败，请检查！！！");
	    	Assert.fail("创建keywordimage，list页面不显示图片，或者创建失败，请检查！！！");
	    }
	    
	    while(true){
	    	b=true;
	    	try{
	    		search = driver.findElement(By.xpath("//div[@id='key-word-grid']/div/table/tbody/tr/td[7]/a")).getText();
	    	}catch(org.openqa.selenium.StaleElementReferenceException e){
	   		 	System.out.println("搜索中，捕捉异常org.openqa.selenium.StaleElementReferenceException");
	   		 	b=false;
	    	}
	    ...
	    try {
	      AssertJUnit.assertEquals("0", search);
	    	...
	    	logger.error("创建keywordimage，list页面触发次数显示不对，或者创建失败，请检查！！！");
	    	Assert.fail("创建keywordimage，list页面触发次数显示不对，或者创建失败，请检查！！！");
	    }
	    ...
	    try {
	    	...
	    	logger.error("创建keywordimage，list页面status显示不对，或者创建失败，请检查！！！");
	      Assert.fail("创建keywordimage，list页面status显示不对，或者创建失败，请检查！！！");
	    }
	    while(true){
	     b = true;
	    	try{
	    ...
	    Thread.sleep(2000);
	    try {
	      AssertJUnit.assertEquals("seleniumimage"+time, driver.findElement(By.cssSelector("li.tagItem")).getText());
	    } catch (Error e) {
