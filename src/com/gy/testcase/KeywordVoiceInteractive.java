...
public class KeywordVoiceInteractive extends TestBase{
	private static Logger logger = Logger.getLogger(KeywordVoiceInteractive.class);
	@Test(dependsOnGroups="keywordVoice")
	  public void keywordVoiceInteractive() throws Exception {
		//接口部分
		//String[] info= {"appid","secret","mid","WechatID","openid"};
		String[] attr=info();
		String[] arr=getSignature(attr[2],attr[0],attr[1]);
		String[] msg={"seleniumvoice"};
					...
					Document doc = DocumentHelper.parseText(keyreply[i]);
				   	...
				   	if(it.hasNext()){
				   		...
				   		while(it.hasNext()){
					   		...
					   		System.out.println("keyword Voice得到的MediaId是"+mediaid);
					   		try{
					   				assertNotNull(mediaid);
					   				System.out.println("keyword Voice触发Voice 成功 ！！！");
					   				
					   		}catch(Error e){
					   			
					   			logger.error("keyword Voice触发keyword Voice失败！！！");
					   			Assert.fail("keyword Voice触发keyword Voice失败！！！");
					   		}
					   	}
				   	
				   	}else{
				   		logger.error("keyword Voice触发失败,xml没找到Voice");
				   		Assert.fail("keyword Voice触发失败,xml没找到Voice");
				   		}
				   	}
		    	}
		}
		...
		//后台验证触发数据	
			    ...
			    new Select(driver.findElement(By.name("KeyWord[replyType]"))).selectByVisibleText("Voice");
			   // driver.findElement(By.name("KeyWord[keyWord]")).sendKeys(Keys.ENTER);
			    ...
			    try {
			    	...
			    	assertTrue(num>=1);
			    	  ...
			    	  logger.error("keyword Voice接口测试触发过后触发次数显示错误");
			        Assert.fail("keyword Voice接口测试触发过后触发次数显示错误");
			      }
			    ...
			    for(int k=1;k<num2+1;k++){
			    	if(isElementPresent(By.xpath("//div[@id='page']/div[2]/div[2]/div[2]/table/tbody/tr["+k+"]/td[1]/a"))){
					    	...
					    	if(nickname.contains("rainbow")){
					    		...
					    		try{
					    			System.out.println("keyword Voice接口测试rainbow触发显示次数是："+ driver.findElement(By.xpath("//div[@id='page']/div[2]/div[2]/div[2]/table/tbody/tr["+k+"]/td[2]/a")).getText());	
					    			assertEquals("1",driver.findElement(By.xpath("//div[@id='page']/div[2]/div[2]/div[2]/table/tbody/tr["+k+"]/td[2]/a")).getText());
					    			k=num2+1;
					    			System.out.println(k);
					    		}catch(Error e){
					    			
					    			logger.error("keyword Voice接口测试rainbow触发过后触发次数显示错误");
					    	        Assert.fail("keyword Voice接口测试rainbow触发过后触发次数显示错误");
					    		}
					    	}
			    		}
			    }
