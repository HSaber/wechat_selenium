...
public class CloseImageTriggerInteractive extends TestBase{
	private static Logger logger = Logger.getLogger(CloseImageTriggerInteractive.class);
	//@Test
	@Test(dependsOnGroups="imageTriggerChinaInteractive")
	  public void closeImageTriggerInteractive() throws Exception {
		
		navigation("Triggers");
	    Thread.sleep(2000);
	    String category = driver.findElement(By.xpath("//div[@id='triggers-grid']/div[1]/table/tbody/tr[1]/td[2]")).getText();
	    if(category.equals("seleniumchina image trigger")){
		    ...
		    while(true){
		    	...
		    	if(category.equals("seleniumchina image trigger")&&b)
		    		break;
		    }
	    }
	   ...
	   new Select(driver.findElement(By.id("Triggers_status"))).selectByVisibleText("Inactive");
	    ...
	    if(category2.equals("seleniumchina image trigger")){
	    	System.out.println("第一行就是seleniumchina image trigger，不需要搜素查询");
	    }else{
	    	driver.findElement(By.name("Triggers[category]")).clear();
		    driver.findElement(By.name("Triggers[category]")).sendKeys("seleniumchina image trigger");
		   ...
		   // new Select(driver.findElement(By.name("Triggers[status]"))).selectByVisibleText("Inactive");
		    while(true){
		    	...
		    	if(category2.equals("seleniumchina image trigger")&&b)
		    		break;
		    }
	    }
	    
	    int a = Integer.parseInt(driver.findElement(By.xpath("//div[@id='triggers-grid']/div[1]/table/tbody/tr/td[8]/a")).getText());
        System.out.println("接口post操作前，CloseImageTriggerInteractive当前总触发次数是"+a+"次");
	    ...
	    try{
  		AssertJUnit.assertEquals("Inactive",status);
	    	 ...
	    	 logger.error("该image trigger更改status未成功！！！");
	    	Assert.fail("该image trigger更改status未成功！！！");
	    }
	    navigation("Triggers");
	    
	  String category4 = driver.findElement(By.xpath("//div[@id='triggers-grid']/div[1]/table/tbody/tr[1]/td[2]")).getText();
	    if(category4.equals("seleniumchina image trigger")){
	    	System.out.println("第一行就是seleniumchina image trigger，不需要搜素查询");
	    }else{
	    	driver.findElement(By.name("Triggers[category]")).clear();
		    driver.findElement(By.name("Triggers[category]")).sendKeys("seleniumchina image trigger");
		   ...
		   // new Select(driver.findElement(By.name("Triggers[status]"))).selectByVisibleText("Inactive");
		    while(true){
		    	...
		    	if(category4.equals("seleniumchina image trigger")&&b)
		    		break;
		    }
	    }
	    int c =0;
        ...
        System.out.println("接口post前，rainbow触发CloseImageTriggerInteractive的次数是:"+c);
	    //接口部分
	  ...
	  String category3 = driver.findElement(By.xpath("//div[@id='triggers-grid']/div[1]/table/tbody/tr[1]/td[2]")).getText();
	    if(category3.equals("seleniumchina image trigger")){
	    	System.out.println("第一行就是seleniumchina image trigger，不需要搜素查询");
	    }else{
	    	driver.findElement(By.name("Triggers[category]")).clear();
		    driver.findElement(By.name("Triggers[category]")).sendKeys("seleniumchina image trigger");
		   ...
		   // new Select(driver.findElement(By.name("Triggers[status]"))).selectByVisibleText("Inactive");
		    while(true){
		    	...
		    	if(category3.equals("seleniumchina image trigger")&&b)
		    		break;
		    }
	    }
	    try {
	        int num = Integer.parseInt(driver.findElement(By.xpath("//div[@id='triggers-grid']/div[1]/table/tbody/tr/td[8]/a")).getText());
	        System.out.println("接口post操作后，CloseImageTriggerInteractive当前总触发次数是"+num+"次");
	    	AssertJUnit.assertTrue(num>=a);
	      } catch (Error e) {
	        //verificationErrors.append(e.toString());
	    	  logger.error("CloseImageTriggerInteractive搜索失败，请检查！");
	        Assert.fail("CloseImageTriggerInteractive搜索失败，请检查！");
	      }
	    ...
	    try {
	      AssertJUnit.assertEquals(c,r);
	    	...
	    	if(r>c){
	    		logger.error("rainbow不应该触发CloseImageTriggerInteractive，勤检查！！");
	    		Assert.fail("rainbow不应该触发CloseImageTriggerInteractive，勤检查！！");
	    	}else{
	    		logger.error("CloseImageTriggerInteractive搜索失败，请检查！");
	    		Assert.fail("CloseImageTriggerInteractive搜索失败，请检查！");
	    	}
