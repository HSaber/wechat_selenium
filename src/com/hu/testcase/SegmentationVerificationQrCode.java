...
public class SegmentationVerificationQrCode {
	 WebDriver driver;
	 private static Logger logger = Logger.getLogger(SegmentationVerificationQrCode.class);
		@BeforeClass
		...
		try{
			...
			+ "where a.wid="+methods.mid+" and a.type='qrcode_scan' and a.created_date>'"+SystemYesterday+"'";	
			...
			try{
				...
				logger.error("Qrcode Date segmentation 查询人数不一致，mysql result:"+QrcodeFollower+"  Segmentation result:"+summary_total);
				e.printStackTrace();
			}
	        ...
	        + "{\"has_child\":{\"type\":\"user_action_record_others\","
			...
			String QrcodeOtherJson=hits.getTotalHits()+"";
			System.out.println("user_action_record_others 查询的记录数="+hits.getTotalHits());
			...
			String QrcodeJson=hits.getTotalHits()+"";
			System.out.println("user_action_record 查询的记录数="+hits.getTotalHits());
			try{
				AssertJUnit.assertEquals(QrcodeJson, QrcodeOtherJson);
			}catch(AssertionError e){
				logger.error("QrcodeDate segmentation 查询人数不一致，user_action_record Json result:"+QrcodeJson+"  user_action_record_others result:"+QrcodeOtherJson);
				e.printStackTrace();
			}
