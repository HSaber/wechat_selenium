...
public class SegmentationVerificationMenu {
	 WebDriver driver;
	 private static Logger logger = Logger.getLogger(SegmentationVerificationMenu.class);
		@BeforeClass
		...
		+ "where a.wid="+methods.mid+" and a.type='menu' and a.created_date>'"+SystemYesterday+"'";	
		...
		try{
			...
			logger.error("MenuDate segmentation 查询人数不一致，mysql result:"+MenuFollower+"  Segmentation result:"+summary_total);
			e.printStackTrace();
		}
		...
		String MenuOtherJson=hits.getTotalHits()+"";
		...
		String MenuJson=hits.getTotalHits()+"";
		System.out.println("user_action_record 查询的记录数="+hits.getTotalHits());
		try{
			AssertJUnit.assertEquals(MenuJson, MenuOtherJson);
		}catch(AssertionError e){
			logger.error("MenuDate segmentation 查询人数不一致，user_action_record Json result:"+MenuJson+"  user_action_record_menu result:"+MenuOtherJson);
			e.printStackTrace();
		}
